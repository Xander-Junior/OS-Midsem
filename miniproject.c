...
typedef struct
{
    unsigned int valid : 1;       // Valid bit: 1 if page is in physical memory, 0 otherwise it's in secondary memory
...
void displayPageAllocation()
{
    ...
    for (int processId = 0; processId < MAX_PROCESSES; processId++)
    {
        ...
        { // Check if the process is active
            ...
            for (int i = 0; i < TOTAL_PAGES; i++)
            {
                if (procPageTable[i].valid)
                { // Check if the page is valid (allocated)
